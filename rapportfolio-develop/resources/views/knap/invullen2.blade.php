...
@section('content')
    <div class="row">
        <div class="col s12 m12 l12">
            ...
            @endif
                <form action="{{route('invullenknapleerkracht2',$id)}} " method="post">
                {{csrf_field()}}
                    ...
                    <table class="bordered">
                        ...
                        @foreach($zelfknap1 as $zknap1)
                            <tr>
                                ...
                                @foreach($cijfer as $c)
                                    <td><input name="zelfknap[{{$zknap1->knapid}}]" type="checkbox"
                                               id="{{$c}}{{$zknap1->knapid}}"
                                               value="{{$c}}" {{ old("zelfknap.$zknap1->knapid") == "$c" ? 'checked='.'"'.'checked'.'"' : '' }}/>
                                        <label for="{{$c}}{{$zknap1->knapid}}"></label></td>
                                @endforeach
                            </tr>
                        @endforeach
                        ...
                        @foreach($zelfknap2 as $zknap2)
                            <tr>
                                ...
                                @foreach($cijfer as $c)
                                    <td><input name="zelfknap[{{$zknap2->knapid}}]" type="checkbox"
                                               id="{{$c}}{{$zknap2->knapid}}"
                                               value="{{$c}}" {{ old("zelfknap.$zknap2->knapid") == "$c" ? 'checked='.'"'.'checked'.'"' : '' }}/>
                                        <label for="{{$c}}{{$zknap2->knapid}}"></label></td>
                                @endforeach
                            </tr>
                        @endforeach
                    </table>
